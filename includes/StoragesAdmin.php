...
class StoragesAdmin extends Controller{
	public function anyIndex(){
		$rs = $this->getTypes();
		set('data',$rs);
		return render('storages/admin_index');
	}
	public function postAdd(){
		$params = array('name'=>h($_POST['name'])
				  );
		
		$rs = $this->db->save('storages',$params);

		if($rs!=0){
			$msg = "Tempat Penyimpanan telah berhasil ditambahkan";
		}else{
	...
	public function postUpdate(){
		...
		$update = $this->db->update($id,'storages',$params);
	...
	public function getDelete($id){
		...
		if($confirm==1){
			$del = $this->db->delete($id,'storages');
			...
			return render('storages/admin_confirm_delete');
		}
	}
	public function getEdit($id){
		...
		return render('storages/admin_edit');
	}
	...
	public function getTypesById($id){
		return $this->db->get($id,'storages');
	}
	public function getTypes(){
		...
		if(strlen($_REQUEST['search']) > 0 && $this->isValid($_REQUEST['search'])){
			...
			$results = $this->db->query("SELECT * FROM storages 
			...
			$results = $this->db->query("SELECT * FROM storages ORDER BY name LIMIT {$start},{$limit};");

			$sql = "SELECT COUNT(id) as total FROM storages";
			$total = $this->db->query($sql);
		}
